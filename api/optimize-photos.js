...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. Пишем прогресс-коммент
    const progressComment = await addComment(taskId, `⏳ Оптимизирую ${photos.length} фото...`);

    // 3. Скачиваем ВСЕ фото в память
    const photoBuffers = [];
    ...
    // Шаг 4: удаляем технический комментарий с архивом (чтобы не висел в задаче)
    ...
    try {
      await pyrusRequest(`/tasks/${taskId}/comments/${progressComment.id}`, {
        method: 'DELETE',
      });
    } catch (e) {
      console.warn('[OPTIMIZE] could not delete progress comment', e.message);
    }
...
async function updateComment(taskId, commentId, text) {
  // Pyrus не поддерживает edit comment, пока пишем новый
