...
async function processFieldPair(task, taskId, pair, startTime) {
  ...
  let archives = [{ name: `photo_archive_${pair.label.replace(/\s+/g, '_')}_${dateStr}.zip`, buffer: zipBuffer }];

  if (zipBuffer.length > PART_SIZE) {
    archives = await splitZipBySize(optimized, PART_SIZE, pair.inputCode);
  }
